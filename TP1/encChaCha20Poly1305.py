...
class encChaCha20Poly1305():
    ...
    def generateSharedKey(self, sSharedSecret, eSharedSecret, salt=b"0"):
        kdf = PBKDF2HMAC(algorithm=hashes.SHA256(),
                         length=(encChaCha20Poly1305.ENCRYPTION_KEY_SIZE + encChaCha20Poly1305.HMAC_KEY_SIZE),
                         salt=salt, iterations=100, backend=self.backend)
    ...
    def mac(self, msg):
        macer = hmac.HMAC(self.shared_key[encChaCha20Poly1305.ENCRYPTION_KEY_SIZE:encChaCha20Poly1305.HMAC_KEY_SIZE],
                          hashes.SHA256(), backend=self.backend)
        macer.update(msg)
    ...
    def keyAgreementE(self, connection):
        ...
        try:
            self.verifySign(
                pk + encodePublicKey(self.public_key) + e_pk_mac_load["e_key"] + encodePublicKey(self.e_public_key),
                sign)
        except InvalidSignature as In:
        ...
        # test confirmation
        try:
            self.verifyMac(b"KC_1_V" + encodePublicKey(self.e_public_key) + e_pk_mac_load["e_key"],
                           e_pk_mac_load["mac"])
        except InvalidSignature as In:
            # connection.send(pickle.dumps({"mac": "mac","signature":"signature"}))
        ...
        # Send mac and sign
        mac_and_sign = {"mac": self.mac(b"KC_1_U" + encodePublicKey(self.e_public_key) + e_pk_mac_load["e_key"]),
                        "signature": self.encrypt(self.sign(
                            encodePublicKey(self.public_key) + pk + encodePublicKey(self.e_public_key) + e_pk_mac_load[
                                "e_key"]))}
        ...
        key_and_mac_and_sig = pickle.dumps(
            {"e_key": encodePublicKey(self.e_public_key),
             ...
             "signature": self.encrypt(
                 self.sign(encodePublicKey(self.public_key) + pk + encodePublicKey(self.e_public_key) + e_pk))}
        )
